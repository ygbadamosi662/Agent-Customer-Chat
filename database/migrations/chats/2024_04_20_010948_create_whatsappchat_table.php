...
class CreateWhatsappChatTable extends Migration
{
    ...
    public function up()
    {
        if (!Schema::connection('chats')->hasTable('whatsapp_chat')) {
            Schema::connection('chats')->create('whatsapp_chat', function (Blueprint $table) {
                $table->id();
                ...
                $table->enum('status', ['pending', 'received', 'sent', 'failed'])
                    ->default('pending')
                    ->nullable();
                $table->string('complaint_id',100)->nullable();
    ...
    {
        Schema::dropIfExists('whatsappchat');
    }
}
